...
## 📌 내일 여기서 시작 (마지막 작업: 2026-09-13)

### 지금 상태 한 줄
**앱은 완성돼 돌아가고 있다.** 배포·로그인·클라우드 저장·홈 화면 설치·서버 푸시까지
전부 끝났고 실발송까지 확인했다. 남은 건 **메일 자동 수집**뿐이다.

### 오늘 끝낸 것
- 구독 추가·수정·삭제가 클라우드에 실제로 저장됨 (기기 간 동기화)
- PWA — 홈 화면 설치 + 오프라인
- 서버 푸시 — 앱을 닫아놔도 **매일 KST 09:00** 에 결제 D-3 / D-1 / 당일 알림

### 🔴 먼저 할 일 2개 (5분)

1. **넷플릭스 결제일 되돌리기** — 테스트로 `2026-09-16` 으로 바꿔 놨다.
   앱에서 실제 날짜로 수정하면 클라우드에도 자동 반영된다.
2. **휴대폰에서 알림 켜기** — 설치한 PWA → 설정 → `결제 알림 켜기`.
   **구독은 기기별**이라 PC 를 켰어도 폰은 따로 켜야 한다.
...
where jobname = 'send-billing-reminders'
order by start_time desc limit 3;
```
`status = 'succeeded'` 면 정상. 실패면 아래 "푸시에서 겪은 함정 3가지"를 먼저 볼 것.

### 다음에 할 수 있는 일 (우선순위 순)

| 순위 | 작업 | 분량 | 메모 |
|---|---|---|---|
| 1 | **네이버 메일 자동전달 테스트** | 5분 | 코드 0줄. 네이버 메일 환경설정 → 자동전달 → Gmail. 되면 끝, 안 되면 파서 보강(반나절) |
| 2 | **Gmail 연동 완성** | 30분 | Google Cloud 3단계 (범위 등록 · Gmail API 활성화 · 테스트 사용자). 아래 "3. Gmail 연동" 참고 |
| 3 | `supabase.js` 죽은 코드 정리 | 1시간 | 100줄쯤. 급하지 않음 |
| — | Capacitor 스토어 배포 | — | Gmail CASA 심사가 벽. 권하지 않음 |

### ⚠️ 절대 잃으면 안 되는 파일

```
D:\작업 2\subclean\vapid-keys.local
```
푸시 키다. 잃으면 키를 새로 만들어야 하고 **모든 기기가 알림을 다시 켜야 한다.**
저장소에는 올라가지 않는다 (`.gitignore` 의 `*.local`).

### 적용된 마이그레이션 (전부 실행 완료)
`001` 스키마 · `002` RLS · `003` service_id 타입 · `004` 구독 필드 ·
`005` authenticated GRANT · `006` 푸시 테이블 · `007` service_role GRANT
